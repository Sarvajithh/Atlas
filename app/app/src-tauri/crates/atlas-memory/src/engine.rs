...
pub struct MemoryEngine {
    ...
    /// Quiz/Flashcard/RevisionPlan persistence (§ Learning subsystem).
    /// Separate from `progress` for the same reason `StudyRepository` is a
    /// separate trait from `LearningProgressRepository` (see that trait's
    /// doc comment).
    study: Arc<dyn StudyRepository>,
...
impl MemoryEngine {
    ...
    pub fn study(&self) -> &Arc<dyn StudyRepository> {
        &self.study
    }
...
mod tests {
    ...
    fn engine_exposes_all_injected_dependencies() {
        ...
        assert!(engine
            .study()
            .list_quizzes_for_workspace(WorkspaceId(1))
            .unwrap()
            .is_empty());
    }
}
