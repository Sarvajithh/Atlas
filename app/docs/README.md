...
5. **Vector storage (§5): `EmbeddedVectorStore` retained for V1.0, not Qdrant/LanceDB.** §5 mandates "Qdrant or LanceDB (embedded/local mode)". The shipped implementation is instead a custom, dependency-free, brute-force cosine-similarity store (`atlas-vector::EmbeddedVectorStore`). This was previously a disclosed-but-unamended deviation (see `README.md`'s "Known Limitations"); it is now a formally reviewed and accepted one for V1.0, per the evaluation in `docs/vector_store_decision.md`. Rationale (full detail in that document): the current store already sits behind the same `VectorStore`/`VectorSearchRepository` interfaces the rest of the app depends on, so a future migration remains a contained, single-file change; O(n) brute-force search is not an observed bottleneck at this app's single-user, offline-first, per-workspace corpus scale (§2.1, §25); and both Qdrant's and LanceDB's Rust dependency trees hit the same MSRV/edition2024 build-toolchain constraint already documented in this file's "Known Environment Limitations" section. §5's table entry is left as written above (the original contract, unmodified) — this entry records the accepted deviation, it does not silently rewrite §5 itself. Revisit if real per-workspace corpus sizes are observed to regularly exceed roughly 50,000–100,000 indexed chunks, or if multi-workspace cross-search becomes a first-class feature.
