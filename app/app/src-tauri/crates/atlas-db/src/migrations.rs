...
const MIGRATIONS: &[Migration] = &[
    ...
    // Learning subsystem structured output (§ implementation plan): Quiz /
    // Flashcard / Revision Planner generation depth, and a real weak-topic
    // aggregate. Purely additive -- no existing table's columns are
    // touched. `questions_json`/`cards_json`/`items_json` store the
    // already-validated (`study_output::parse_*`) structured payload as
    // JSON, keyed by the same workspace/document/topic tagging pattern
    // used elsewhere (documents, chat_sessions); `document_id` is
    // nullable since a quiz/flashcard set can be generated for a whole
    // workspace/topic rather than one document. `quiz_topic_stats` is the
    // incrementally-updated correctness aggregate `list_weak_topics`
    // reads from -- a real computation, not something re-derived by the
    // model each time (see `AnalyticsRepository::record_quiz_answer`).
    Migration {
        id: "0017_create_quiz_flashcard_revision_plan",
        sql: "
            ...
            CREATE INDEX IF NOT EXISTS idx_flashcard_sets_workspace ON flashcard_sets(workspace_id);
            CREATE INDEX IF NOT EXISTS idx_flashcard_sets_document ON flashcard_sets(document_id);
            ...
            CREATE INDEX IF NOT EXISTS idx_revision_plans_workspace ON revision_plans(workspace_id);
