...
use atlas_indexer::job_queue::{JobQueue, JOB_TYPE_EXTRACT_CONCEPTS, JOB_TYPE_INDEX_DOCUMENT};
...
pub struct IndexingWorker {
    ...
    /// Concept Graph extraction (§20, Phase 5). `None` is a valid,
    /// supported configuration (e.g. a test harness that only cares about
    /// indexing) -- a job of type `extract_concepts` is then recorded as a
    /// recoverable failure rather than panicking, exactly like any other
    /// missing-dependency case this worker already handles.
...
impl IndexingWorker {
    ...
    /// Attaches the Concept Graph engine (§20). Kept as a builder step
    /// rather than a `new` parameter so existing callers/tests that never
    /// touch the Concept Graph are unaffected.
...
fn process_job(
    ...
    match job.job_type.as_str() {
        JOB_TYPE_INDEX_DOCUMENT => {
            process_index_job(workspace_engine, indexing_pipeline, job_queue, events, job)
        }
        JOB_TYPE_EXTRACT_CONCEPTS => {
            process_extract_concepts_job(indexing_pipeline, job_queue, graph_engine, job)
        }
        other => {
            // §28: the job type space is extensible; an unrecognized type
            // from a future job producer is a recoverable, visible
            // failure, not a silent drop or a panic (§45.2).
            let _ = job_queue.mark_failed(job.id, format!("unrecognized job type '{other}'"));
        }
    }
}

fn process_index_job(
    workspace_engine: &Arc<WorkspaceEngine>,
    indexing_pipeline: &Arc<IndexingPipeline>,
    job_queue: &Arc<JobQueue>,
    events: &Arc<dyn EventBus>,
    ...
    match outcome {
        Ok(index_outcome) => {
            ...
            // Concept Graph extraction (§20, Phase 5): queued as its own
            // async job -- appended here, after indexing succeeds with at
            // least one real chunk -- rather than run inline, so a slow or
            // failing extraction call can never affect this job's own
            // success/failure or hold up the indexing worker's throughput.
            if let IndexOutcome::Indexed { chunk_count } = index_outcome {
                if chunk_count > 0 {
                    if let Some(document) = indexing_pipeline
                        .documents()
                        .list_for_workspace(workspace_id)
                        .ok()
                        .and_then(|docs| docs.into_iter().find(|d| d.relative_path == relative_path))
                    {
                        if let Err(err) =
                            job_queue.enqueue_extract_concepts_job(workspace_id, document.id)
                        {
                            log_warn!(
                                "failed to enqueue concept extraction job for document {}: {err}",
                                document.id.0
                            );
                        }
                    }
...
fn process_extract_concepts_job(
    indexing_pipeline: &Arc<IndexingPipeline>,
    job_queue: &Arc<JobQueue>,
    graph_engine: Option<&Arc<GraphEngine>>,
    job: Job,
) {
    let (workspace_id, document_id) = match parse_extract_payload(&job) {
        Ok(pair) => pair,
        Err(err) => {
            let _ = job_queue.mark_failed(job.id, err.message);
            return;
    ...
    let Some(graph_engine) = graph_engine else {
        // No extraction model wired (e.g. a test harness, or a running app
        // with no Reasoning-role model assigned yet). This is Recoverable,
        // not a defect in this job: mark it failed (subject to the same
        // bounded retry as any other job) rather than silently dropping it
        // or panicking.
        let _ = job_queue.mark_failed(
            job.id,
            "concept extraction skipped: no GraphEngine/model wired into this worker".to_string(),
        );
        return;
    };
    ...
    let outcome = indexing_pipeline
        .documents()
        .find_by_id(document_id)
        .and_then(|maybe_doc| {
            maybe_doc.ok_or_else(|| {
                AppError::indexing(format!("document {} no longer exists", document_id.0))
            })
        })
        .and_then(|document| {
            let chunks = indexing_pipeline.chunks().list_for_document(document_id)?;
            let chunk_texts: Vec<String> = chunks.into_iter().map(|c| c.text_content).collect();
            graph_engine.extract_for_document(workspace_id, &document.relative_path, &chunk_texts)
        });

    match outcome {
        Ok(result) => {
            log_info!(
                "concept extraction for document {}: {} nodes created, {} reused, {} edges created",
                document_id.0,
        ...
        Err(err) => {
            // A failed extraction (model unreachable, persistently
            // unparseable output, etc.) is Recoverable (§45.1): it must
            // never affect the document's own `Parsed`/`ParsedEmpty`
            // status, which `process_index_job` already finalized before
...
fn parse_extract_payload(job: &Job) -> Result<(WorkspaceId, DocumentId), AppError> {
    ...
    let document_id = job
        ...
        .get("document_id")
        .and_then(|v| v.as_i64())
        .ok_or_else(|| AppError::indexing(format!("job {}: payload missing 'document_id'", job.id.0)))?;
    Ok((WorkspaceId(workspace_id), DocumentId(document_id)))
}
...
mod tests {
    ...
    fn worker_drains_a_queued_job_into_a_parsed_document() {
        ...
        for _ in 0..100 {
            ...
            let queued_drained = job_queue.repository().list_by_status(JobStatus::Queued).unwrap().is_empty();
            let indexed = job_queue.repository().list_by_status(JobStatus::Succeeded).unwrap().len() == 1;
            // With no `GraphEngine` wired into this worker (§20), the
            // `extract_concepts` job the index job enqueues on success is
            // expected to reach `Failed` (recoverable, no model attached)
            // rather than `Succeeded` -- so "drained" here means "nothing
            // left queued", not "everything succeeded".
            if indexed && queued_drained {
                succeeded = true;
                break;
            }
        }
